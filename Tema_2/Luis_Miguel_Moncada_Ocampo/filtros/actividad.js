...
function greyConverter() {
    ...
    for (x = 0; x < pixels.length; x++) {
        for (y = 0; y < pixels[x].length; y++) {
            let media = (pixels[x][y].reduce((previous, current) => current += previous)) / pixels[x][y].length;
...
function blackAndWhiteConverter() {
    ...
    for (x = 0; x < pixels.length; x++) {
        for (y = 0; y < pixels[x].length; y++) {
            let media = (pixels[x][y].reduce((previous, current) => current += previous)) / pixels[x][y].length;
            pixels[x][y] = [media < 128 ? 0 : 255, media < 128 ? 0 : 255, media < 128 ? 0 : 255];
        }
    }
...
function scaleDown() {
    ...
    //Aqui tu codigo
    let newPixels = [];
    for (let x = 0; x < pixels.length; x = x + 2) {
        let nuevaFila = [];
        for (let y = 0; y < pixels[x].length; y = y + 2) {
            nuevaFila.push(pixels[x][y]);
        }
...
function dimBrightness(dimFactor) {
    ...
    for (x = 0; x < pixels.length; x++) {
        for (y = 0; y < pixels[x].length; y++) {
            pixels[x][y] = [pixels[x][y][0] / dimFactor, pixels[x][y][1] / dimFactor, pixels[x][y][2] / dimFactor];
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    //Aqui tu codigo
    for (x = 0; x < catPixels.length; x++) {
        let nuevaFila = [];
        for (y = 0; y < catPixels[x].length; y++) {
            let pixel = [(catPixels[x][y][0] * alphaSecond) + (dogPixels[x][y][0] * alphaFirst), (catPixels[x][y][1] * alphaSecond) + (dogPixels[x][y][1] * alphaFirst), (catPixels[x][y][2] * alphaSecond) + (dogPixels[x][y][2] * alphaFirst)];
            nuevaFila.push(pixel);
        }
