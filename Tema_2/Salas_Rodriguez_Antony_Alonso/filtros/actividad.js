...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      prom = (pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2])/3
      pixels[i][j][0] = pixels[i][j][1] = pixels[i][j][2] = prom
    }
  }
  handler.savePixels(pixels, outputPath);
}
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      if (prom < 128){
        ...
        pixels[i][j][0] = pixels[i][j][1] = pixels[i][j][2] = 255
      }
    }
  }
  handler.savePixels(pixels, outputPath);
...
function scaleDown() {
  ...
  let pixels = handler.getPixels();
  pixels = pixels.filter((item, index) => index % 2 === 0)
  for (let i = 0; i < pixels.length; i++) {
    pixels[i] = pixels[i].filter((item, index) => index % 2 === 0)
  }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    pixels[i] = catPixels[i]
    for (let j = 0; j < catPixels[i].length; j++) {
      pixels[i][j][0] = (dogPixels[i][j][0]*alphaFirst)  + (catPixels[i][j][0]*alphaSecond)
      pixels[i][j][1] = (dogPixels[i][j][0]*alphaFirst)  + (catPixels[i][j][1]*alphaSecond)
      pixels[i][j][2] = (dogPixels[i][j][0]*alphaFirst)  + (catPixels[i][j][2]*alphaSecond)
