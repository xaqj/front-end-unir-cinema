...
function redConverter() {
  ...
  //Aqui tu codigo
  pixels.forEach(function(pixelRow){
...
function scaleDown() {
  ...
  pixels.forEach(function(pixelRow, i){
    if (i % 2 === 0) {
      let newPixelRow = [];
      pixelRow.forEach(function(pixel, j){
        if (j % 2 === 0){
          newPixelRow.push(pixel);
        }
      })
      newPixels.push(newPixelRow);
    }
  })
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < dogPixels.length; i++){
    ...
    for (let j = 0; j < dogPixels[i].length; j++){
      let red = dogPixels[i][j][R] * alphaFirst + catPixels[i][j][R] * alphaSecond;
      let green = dogPixels[i][j][G] * alphaFirst + catPixels[i][j][G] * alphaSecond;
      let blue = dogPixels[i][j][B] * alphaFirst + catPixels[i][j][B] * alphaSecond;
      pixels[i].push([red, green, blue]);
    }
  }
